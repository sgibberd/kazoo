...
%    io:format("Pending: ~p~n",[NPending]),
...
%    io:format("Got UDP:~n"),
    case eradius_lib:decode_request_id(EncRequest) of
        {ReqId, EncRequest} ->
%	    io:format("Looking for ~p ~p ~p~n",[FromIP,FromPort,ReqId]), 
            case dict:find({FromIP, FromPort, ReqId}, Pending) of
                error ->
                    ...
                    {noreply, State};
                {ok, WaitingSender} ->
%		    io:format("Found - sending to ~p: ~p ~p ~p~n",[WaitingSender,self(),ReqId,EncRequest]),
