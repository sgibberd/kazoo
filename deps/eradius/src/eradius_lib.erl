...
-export([random_authenticator/0, zero_authenticator/0, pad_to/2, set_attr/3, set_attributes/2,new_radius_request/0,new_acc_req/0,acct_status/1]).
...
-spec new_radius_request() -> #radius_request{}.
new_radius_request() ->
    #radius_request{}.

acct_status(start) ->  ?RStatus_Type_Start;
acct_status(stop) -> ?RStatus_Type_Stop.
...
encode_attribute(Req, #attribute{type=Type, id = ID, enc = Enc}, Value) when
      ID >= ?Digest_Realm andalso ID =< ?Digest_User_Name ->
    InnerVal = encode_attribute(Req,#attribute{type=Type, 
					       id=ID -?Digest_Realm + 1,
					       enc=no},Value),
    encode_attribute(Req, #attribute{type=binary,id = ?Digest_Attributes,
				     enc=Enc},InnerVal);
